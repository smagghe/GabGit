...
void addMenu(String& str)
{
  ...
  {
    str += F("<style>");
    str += F("* {font-family:sans-serif; font-size:8pt;}");
    ...
    str += F(".button-link {padding:5px 5px; background-color:#0077dd; color:#fff; border:solid 1px #fff; text-decoration:none}");
    str += F(".button-menu:hover {background:#ddddff;}");
    str += F(".button-link:hover {background:#369;}");
    str += F("th {padding:5px; background-color:black; color:#ffffff;}");
    str += F("td {padding:7px;}");
    str += F("table {color:white;}");
    str += F(".div_devices {position:absolute;top:100px;float: left;width:75%;}");
    str += F(".div_edit {float: right;}");
...
void handle_devices() {
  ...
  reply += F("<div id='div_devices'><table cellpadding='4' border='1' frame='box' rules='all'><TH>");
  ...
  if (index != 0)
  {
    ...
    reply += F("<div id='div_edit'><form name='frmselect' method='post'><table><TH>Task Settings<TH>Value");

    reply += F("<table><TR><TD>Device:<TD>");
